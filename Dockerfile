...
RUN ["/syft", "--output", "spdx-json=/whisper-to-graphite.spdx.json", "/usr/bin/whisper-to-graphite"]

FROM cgr.dev/chainguard/static:latest
WORKDIR /tmp
COPY --from=builder /workspace/whisper-to-graphite /usr/bin/
COPY --from=sbomgen /whisper-to-graphite.spdx.json /var/lib/db/sbom/whisper-to-graphite.spdx.json
